...
const applyHardMode = (state) => {
  const nextState = copy(state);

  nextState.player.hp--;
  log('Hard mode penalty deals 1 damage.');

  return checkGame(nextState);
};
...
const permute = (state, winningGames = []) => {
  ...
  // apply hard mode debuff before each round
  state = checkGame(applyHardMode(state));

  // exit if no more spells can be cast
  if (state.player.mana < spells.magicMissile.cost) {
    return winningGames;
  }
  ...
  for (let i = 0; i < activeGames.length; i++) {
    const currentGame = activeGames[i];

    winningGames = [
      ...winningGames,
      ...permute(currentGame, winningGames),
    ];
  }
